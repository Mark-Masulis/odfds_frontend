...
const columns = [
  ...
  {
    field: 'restaurantName', headerName: 'Restaurant Name', width: 120, sortable: false, valueGetter: (params) => params.row.restaurant.name
  },
  {
    field: 'restaurantPhone', headerName: 'Restaurant Phone', width: 150, sortable: false, valueGetter: (params) => params.row.restaurant.phone
  },
  {
    field: 'restaurantAddress', headerName: 'Restaurant Address', width: 450, sortable: false, valueGetter: (params) => params.row.restaurant.street + ", " + params.row.restaurant.city + ", " + params.row.restaurant.state + " " + params.row.restaurant.zipCode  
...
export default function RestaurantOrders(props) {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await fetch(process.env.REACT_APP_API + "/driver/orders?pageSize=" + paginationModel.pageSize + "&page=" + (paginationModel.page + 1), {
        method: "GET",
        headers: {
